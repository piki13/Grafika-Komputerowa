 ...
 * 
 * This class is identical to the solution for Exercise 13.2
 * in David Eck's "Introduction to Programming using Java"
 * ( http://math.hws.edu/eck/cs124/javanotes7/c13/ex2-ans.html ),
 * except for a new name and the addition of the copyOSC() method
 * and the installImage() method.
 */
public class PaintPanel extends JPanel {

	/** Returns a BufferedImage containing a copy of the current
	 *  image in the panel.  This is a copy of the "off-screen
	 *  canvas" in which the image is stored.
	 */
	public BufferedImage copyOSC() {
		BufferedImage img;
		img = new BufferedImage( OSC.getWidth(), OSC.getHeight(), BufferedImage.TYPE_INT_ARGB);
		Graphics2D g = img.createGraphics();
		g.drawImage( OSC, 0, 0, OSC.getWidth(), OSC.getHeight(), null);
		g.dispose();
		return img;
	}
	
	/**
	 * Replaces the current image in this panel with a given BufferedImage.
	 * The new image is resized, if necessary, to exactly fill the panel.
	 * This action is undoable.
	 */
	public void installImage( BufferedImage img ) {
		BufferedImage temp = imageSavedForUndo;
		imageSavedForUndo = OSC;
		OSC = temp;
		Graphics2D g = OSC.createGraphics();
		g.drawImage(img, 0, 0, OSC.getWidth(), OSC.getHeight(), null);
		repaint();
		g.dispose();
	}
	...
	private void putCurrentShape(Graphics g) {
		...
		case STROKED_FILLED_OVAL:
			...
			break;
		default: //  STROKED_FILLED_RECT is the only other possibility
	...
	private class MenuHandler implements ActionListener {
		public void actionPerformed(ActionEvent evt) {
			...
			if (command.equals("Custom Drawing Color...")) {
				Color newColor = JColorChooser.showDialog(PaintPanel.this, 
