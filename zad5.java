 ...
 * stereo.
 */
public class zad5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private int objectNumber = 2;        // Which object to draw (1 ,2, 3, 4, 5, or 6)?
	                                     //   (Controlled by number keys.)

	private boolean useAnaglyph = false; // Should anaglyph stereo be used?
	                                     //    (Controlled by space bar.)

	private int rotateX = 1;    // Rotations of the cube about the axes.
	 ...
	 */
	private void scianka(float j, float k, float n, GL2 gl2) {
    ...
    private void korkociag(double n, GL2 gl2) {
        ...
        gl2.glColor3f((float) 0, 1, 0); // darkred
        while (x <= t) {
            nastKat = (kat * pi / 180);
...
//        gl2.glRotatef(rotateZ, 1, 0, 0);   // Apply rotations to complete object.
//        gl2.glRotatef(rotateY, 0, 1, 0);
//        gl2.glRotatef(rotateX, 0, 0, 1);
